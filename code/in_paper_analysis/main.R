...
hyp_crime <- events_df_tmp2[table_group=='crime',]
hyp_week43 <- events_df_tmp2[table_group=='week_43',]
hyp_terror_attack <- events_df_tmp2[table_group=='terrorist_attack',]
hyp_election <- events_df_tmp2[table_group=='election',]
hyp_restriction <- events_df_tmp2[table_group=='restriction',]
...
int_events <- hyp_terror_attack[event_names!='drottninggatan',]
int_terror <- bayes_fixed_effect_avg(frame_type = 'security', no_compare_frame = '',
                                 event_data = int_events, profile_data = df_profile,
                                 pre_time_window = pre_weekly_tw, post_time_window = post_weekly_tw,
                                 pre_nr_weeks = pre_weeks_nr,  post_nr_weeks = post_weeks_nr,
                                 n_chains = 10, n_iter = 10000)
...
open_hearts <- bayes_fixed_effect_avg(frame_type = 'humanitarian', event_data = hyp_human_rights[event_names%in%c('open_hearts'),],
                                  profile_data = df_profile,   pre_time_window = pre_weekly_tw, no_compare_frame = '',
                                  post_time_window = post_weekly_tw, pre_nr_weeks = pre_weeks_nr,
                                  post_nr_weeks = post_weeks_nr, n_chains = 10, n_iter = 10000)
gc()
...
# week 43
week43 <- bayes_fixed_effect_avg(frame_type = 'economy',  event_data = hyp_week43,
                             profile_data = df_profile,  pre_time_window = pre_weekly_tw,
                             post_time_window = post_weekly_tw, pre_nr_weeks = pre_weeks_nr,
                             post_nr_weeks = post_weeks_nr, no_compare_frame = 'humanitarian',
                             n_chains = 10,  n_iter = 10000)
gc()

# election
sd_election <- bayes_fixed_effect_avg(frame_type = 'politics', event_data = hyp_election,
                                  profile_data = df_profile,   pre_time_window = pre_weekly_tw,
                                  post_time_window = post_weekly_tw,  pre_nr_weeks = pre_weeks_nr,
                                  post_nr_weeks = post_weeks_nr, no_compare_frame = '',
                                  n_chains = 10, n_iter = 10000)
gc()
# crime
crimes <- bayes_fixed_effect_avg(frame_type = 'security',   event_data = hyp_crime,
                                 profile_data = df_profile,  pre_time_window = pre_weekly_tw,
                                 post_time_window = post_weekly_tw,  pre_nr_weeks = pre_weeks_nr,
                                 post_nr_weeks = post_weeks_nr,  no_compare_frame = '',
                                 n_chains = 10, n_iter = 10000)
