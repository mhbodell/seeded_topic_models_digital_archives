...
prior_words <- as.vector(as.matrix(prior_words_file[1,-1]))
...
for(ti in 1){
  ...
  if(!any(base_files%in%'r_never_with_priors_z3.csv')){
    message('Create: r_never_with_priors_z3.csv')
    ...
    fwrite(smoothed_words_to_check, paste0(base_path,topicnames[ti],'_r_never_with_priors_z3.csv'))
    
    
  } else{
    message('Already exists: r_never_with_priors_z3.csv')
  }
  ...
  if(topicnames[ti]=='immigration'){
    ...
    message('Creating: ts_doc_z3.csv')
    ...
    fwrite(ts_immigration_doc, paste0(base_path,topicnames[ti],'_ts_doc_z3.csv'))
  } else {
    ...
    #} else {
    #  message('Already existed: ts_doc.csv')
    #  ts_immigration_doc <- fread(paste0(base_path,topicnames[ti],'_ts_doc.csv'))
  }
} 
...
if(!any('df_y_z3.csv'%in%base_files)){
  message('Creating: df_y_z3.csv')
  ...
  fwrite(df_y, paste0(base_path, 'df_y_z3.csv'))
  
} else {
  message('Already exists: df_y_z3.csv')
  df_y <- fread(paste0(base_path, 'df_y_z3.csv'))
}
...
pscb_raw <-  ggplot(df_y_inv) +
  ...
  facet_wrap(~topic)
pscb_raw
ggsave(paste0(base_path,"scb_yearly_raw_z3.png"), plot = pscb_raw, limitsize = T, height = 350, width = 500, units = 'mm')
...
pscb <- ggplot(df_y_inv) +
  ...
  facet_wrap(~topic)
pscb
ggsave(paste0(base_path,"scb_yearly_standardized_z3.png"), plot = pscb, limitsize = T, height = 350, width = 500, units = 'mm')
...
if(!any(base_files%like%'df_weekly_z3.csv')){
  message('Creating: df_weekly_z3.csv')
  ...
  fwrite(df_weekly, paste0(base_path, 'df_weekly_z3.csv'))
  
} else {
  df_weekly <- fread(paste0(base_path, 'df_weekly_z3.csv'))
}
...
# using xrank instead
ggsave(paste0(base_path,"weekly_yearly_tot_outliers_z3.png"), plot = pw, limitsize = T, height = 350, width = 500, units = 'mm')
...
pw_early <- df_plot3 %>%
  ...
  guides(size = FALSE,
         ...
         shape = FALSE)
pw_early
ggsave(paste0(base_path,"weekly_yearly_outliers_pre90_z3.png"), plot = pw_early, limitsize = T, height = 350, width = 500, units = 'mm')
fwrite(df_plot3, file = paste0(base_path,'weekly_data_z3.csv'), row.names = F)




if(!any(base_files%like%'immigration_dt_z3.csv')){
  message('Creating: immigration_dt_z3.csv')
  ...
  gc()
  dt <- rbindlist(dt, fill = T)
  fwrite(dt, paste0(base_path, 'immigration_dt_z3.csv'))
